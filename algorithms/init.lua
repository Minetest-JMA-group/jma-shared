...
local raw_execute = algorithms.execute
algorithms.execute = function(argv_table)
	local modname = minetest.get_current_modname()
	if not trusted_mods[modname] then
		return "","[algorithms]: Permission denied, not a trusted mod", 1
	end
	return raw_execute(argv_table)
end
