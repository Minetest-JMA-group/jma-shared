...
local kick_message =
	"You must accept the Terms of Service to play on this server.\nSince you have declined, you cannot join at this time.\nIf you change your mind, please reconnect and accept the TOS. Send an e-mail to support.jma@proton.me including all your usernames to request a deletion of your data."
local storage = core.get_mod_storage()
local pending_confirmations = {}
local confirmation_timeout = 10

if not storage:contains("tos_version") then
	storage:set_int("tos_version", 1)
end
...
function jma_greeter.show_tos(player)
	...
	if jma_greeter.has_accepted_tos(player) then
		...
		fs = fs
			.. "textarea[0.1,0.9;7.8,0.6;tos_url;;"
			.. tos_url
			.. "]"
			.. "button_url[2.5,2.3;3,1;tos_button;Open TOS;"
			.. tos_url
			.. "]"
			.. "button_exit[2.5,3.5;3,1;ok;Okay]"
	else
