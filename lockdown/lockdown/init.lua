	...
	if state then
		...
		if duration and duration > 0 then
			...
			modstorage:set_int("lockdown_until", lockdown_until)
				algorithms.time_to_string(duration))
		else
			lockdown_until = 0
	...
	func = function(name, param)
		...
		if mode:find("y") then
			...
			if time > 0 then
				...
				core.log("action", "[lockdown] Player " .. name .. " enabled lockdown with timeout")
				return true, "Lockdown enabled for " .. algorithms.time_to_string(time)
			else
				lockdown.set(true)
			...
			if lockdown.is_enabled() then
				if lockdown_until > 0 then
					return true, "Lockdown active (" ..
						algorithms.time_to_string(math.abs(lockdown_until - os.time())) .. " left)"
				else
